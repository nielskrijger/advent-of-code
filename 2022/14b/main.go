...
func main() {
	grid, maxY := loadGrid("2022/14b/sample.txt")

	fmt.Printf("Answer: %d", fallingSand(grid, maxY+2))
}
	...
	for {
		...
		for {
			for i, drop := range moves {
				if _, ok := grid[sand.Add(drop)]; !ok && sand.Y < bottomY-1 {
					sand = sand.Add(drop)
					break
				}
				...
				if i == len(moves)-1 { // Exhausted all options, sand can't fall any further
					stillSandCount++
					grid[sand] = 2 // Sand can't fall any further
					if sand == image.Pt(500, 0) {
